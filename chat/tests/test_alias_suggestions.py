...
class AliasParsingTests(SimpleTestCase):
    ...
    def test_generator_uses_shared_inference_slot_and_bounded_context(self):
        local_model = MagicMock()
        inference = local_model.inference
        ...
        result = LocalAliasGenerator(local_model).generate(context)
...
class AliasSuggestionServiceTests(TestCase):
    ...
    def test_unsaved_form_aliases_are_not_suggested_again(self):
        generator = StaticAliasGenerator([
            "Bella da casa",
            "Bella guardiã",
        ])

        suggestions = AliasSuggestionService(generator).suggest(
            self.dog,
            additional_aliases="Bella da casa",
        )

        self.assertEqual(suggestions, ("Bella guardiã",))
        self.assertIn("Bella da casa", generator.context.existing_aliases)

    def test_rejected_model_output_uses_safe_localized_questions(self):
        generator = StaticAliasGenerator([
            "Bella",
            "German Shepherd",
            "Luna",
    ...
    def test_faq_suggestions_are_paraphrases_not_existing_search_terms(self):
        ...
        save_aliases(faq, "How do reservations work?")
        delivery_faq = FrequentlyAskedQuestion.objects.create(
            question="Is delivery available?",
            answer="Contact us about delivery.",
        )
        save_aliases(delivery_faq, "Do you deliver puppies?")
        generator = StaticAliasGenerator([
...
class AliasSuggestionAdminTests(TestCase):
    ...
    def test_post_returns_suggestions_without_persisting_them(self):
        ...
        with patch("chat.admin_aliases.get_chat_runtime") as runtime:
            runtime.return_value.alias_suggestion_service.suggest.return_value = (
                "Bellinha",
            )
        ...
        self.assertEqual(aliases_for(self.dog), "Bela")

    def test_normal_admin_save_persists_aliases_in_the_central_index(self):
        request = RequestFactory().post("/")
        request.user = self.admin_user
        model_admin = admin.site._registry[Animal]
        form = Mock(cleaned_data={
            "chat_search_aliases": "Bela\nBellinha\nBela",
        })

        model_admin.save_model(request, self.dog, form, change=True)

        self.assertEqual(aliases_for(self.dog), "Bela\nBellinha")
        self.assertNotIn(
            "chat_search_aliases",
            {field.name for field in Animal._meta.fields},
        )
    ...
    def test_model_preparation_failure_is_safe_and_actionable(self):
        ...
        with patch("chat.admin_aliases.get_chat_runtime") as runtime:
            runtime.return_value.alias_suggestion_service.suggest.side_effect = (
                ModelPreparing("loading")
            )
            response = self.client.post(self.suggestion_url)
